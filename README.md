...
**Live site:** https://montassarsallami122-spec.github.io/pmp-exam-simulator/

## Managing student access

Accounts live in **`data/users.json`**. Each entry is a student login:

```json
{"u":"username","h":"<sha256 hash of the password>","name":"Display Name"}
```
...
2. Type a display name, username, and password → click **Generate**.
...
5. Give the client their **username + password** and the site link.

### To remove access
Delete that student's line from `data/users.json` and push.

### To reset a password
Generate a new line for the same username and replace the old one.

## How progress works
Each student's attempts and best scores are saved in **their own browser**
(localStorage, namespaced per username). It is private to their device and is
